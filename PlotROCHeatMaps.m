...
% Shank exp by behav
    ...
    for k = 1:(numel(xlabels)/2)-1
        ...
        XZBoxPlot({KOfrac{i}(:,k),KOfrac{i}(:,k+nbehav/2),HETfrac{i}(:,k),HETfrac{i}(:,k+nbehav/2),WTfrac{i}(:,k),WTfrac{i}(:,k+nbehav/2)},...
            [1,2,3,4,5,6],[1,4,2,5,3,6],{'KO-act','KO-sup','HET-act','HET-sup','WT-act','WT-sup'},colors,gca,'dot');
        if i<3
            title(expBvs{k});
        else
            title(catBvs{k});
    ...
    for j = 1:numel(allPairs{i}) 
        ...
        for ff = {'self','derived_self'}
           ...
           % saveas(gcf,['ROC_overlap\',cur_mou.ExperimentID,'\',cur_mou.AnimalID,'-',ff{1},'.png']);
            if strcmp(ff{1},'self')
                indi_act_act{find(strcmp(gens,cur_mou.GenType))} = cat(3,indi_act_act{find(strcmp(gens,cur_mou.GenType))},overlap);
            elseif strcmp(ff{1},'derived_self')
                cat_act_act{find(strcmp(gens,cur_mou.GenType))} = cat(3, cat_act_act{find(strcmp(gens,cur_mou.GenType))}, overlap);
            end
        end
    end
end
...
figure,heatMap(round(mean(indi_act_act{3}(1:end-1,1:end-1,:),3,'omitnan'),2), 'xTickLabels', indiNames(1:end-1),'yTickLabels',indiNames(1:end-1),'fontSize',10);
figure,heatMap(round(mean(cat_act_act{3}(1:end-1,1:end-1,:),3,'omitnan'),2), 'xTickLabels', catNames(1:end-1),'yTickLabels',catNames(1:end-1),'fontSize',10);
